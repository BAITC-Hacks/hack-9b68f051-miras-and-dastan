...
import PreviewBacktest from "../components/PreviewBacktest";
import PreviewNavigation, { usePreviewNavigation, type PreviewView } from "../components/PreviewNavigation";
import navigationStyles from "../components/PreviewNavigation.module.css";
import PurchaseApiPanel from "../components/PurchaseApiPanel";
import AgentWorkspace from "../components/openai/AgentWorkspace";
import previewStyles from "../components/PreviewBacktest.module.css";
...
export default function Home() {
  ...
  const { view, navigate } = usePreviewNavigation();
  const [dataSource, setDataSource] = useState("");
  const closeBacktest = () => changeView('overview');
  ...
  useEffect(() => {
    const closeOverlays = () => {
      setSelected(null); setShowOrder(false); setShowUpload(false);
    };
  ...
  function resetForNewData() {
    setFilter("Все"); setSearch(""); setScenario(initialScenario);
    setSelected(null); setShowOrder(false); setAgentReply("");
    setShowUpload(false); setFileRows([]); setMapping({}); setFileName("");
    navigate('overview');
  }
  function loadDemo() {
    setDataset(makeDemoData()); setDataSource("Демо · 32 SKU / 36 недель");
    resetForNewData(); setNotice("Демо-данные загружены: 32 SKU и 36 недель истории");
  }
  ...
  function useImportedData() {
    ...
    setDataSource(`Файл · ${fileName}`); resetForNewData(); setNotice(`Импортировано ${accepted.length} валидных строк. Начальные остатки импорта: 0; поставщики доступны только для совпавших SKU демо.`);
  }
  ...
  if (view === 'start' || !dataset) return <main className={`landing ${dataset || showUpload ? navigationStyles.landingWithSession : ''}`}>
    ...
    <section className="hero"><div className="eyebrow"><Sparkles size={14} /> Логистика · Электрокомплект</div><h1>Закупки без<br /><em>дорогих ошибок.</em></h1><p>Автономный агент отделяет разовые всплески от реального спроса и готовит объяснимые заказы поставщикам.</p><div className="hero-actions"><button className="button primary" onClick={loadDemo}><Play size={17} fill="currentColor" /> Загрузить демо-данные</button><label className="button secondary"><Upload size={17} /> Загрузить Excel / CSV<input type="file" accept=".xlsx,.xls,.csv" onChange={onFile} /></label></div><div className={navigationStyles.resume}>{dataset && <><button type="button" className="button secondary" onClick={() => changeView('overview')}>Продолжить работу <ChevronRight size={16} /></button><p>Текущая сессия сохранена · {dataSource}</p></>}</div><div className="proof"><span><CircleCheck size={15} /> Детерминированные расчёты</span><span><CircleCheck size={15} /> Без API-ключа</span><span><CircleCheck size={15} /> 32 SKU · 36 недель</span></div></section>
    {notice && <div className={navigationStyles.landingNotice}><div className="notice" role="status">{notice}<button aria-label="Закрыть сообщение" onClick={() => setNotice("")}><X size={15} /></button></div></div>}
    {showUpload && <div className={navigationStyles.importArea}>{uploadPanel}</div>}
  ...
  return <main className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark"><Boxes size={18} /></span><span>StockPilot <b>AI</b></span></div><div className="workspace">Рабочее пространство</div><nav>
      <a href="#overview" className={view === 'overview' ? "nav-active" : ""} aria-label="Обзор" onClick={event => { event.preventDefault(); changeView('overview'); }}><Zap size={17} /> Обзор</a>
      <a href="#backtest" className={view === 'backtest' ? "nav-active" : ""} aria-label="Проверка на прошлом" onClick={event => { event.preventDefault(); changeView('backtest'); }}><Play size={17} /> Проверка на прошлом</a>
      <a href="#api" className={view === 'api' ? "nav-active" : ""} aria-label="API-анализ" onClick={event => { event.preventDefault(); changeView('api'); }}><Bot size={17} /> API-анализ</a>
      <a href="#orders" aria-label="Черновики заказов" onClick={event => { event.preventDefault(); setShowOrder(true); }}><ShoppingCart size={17} /> Черновики заказов</a>
      <a href="#start" aria-label="Выбор данных" onClick={event => { event.preventDefault(); changeView('start'); }}><Upload size={17} /> Выбор данных</a>
    </nav><div className="sidebar-bottom"><div className="agent-card"><span><Bot size={17} /> Агент закупок</span><p>Все числа проверены детерминированными функциями.</p><i><span /> Локальный предпросмотр</i></div><button className="text-button" onClick={loadDemo}>↻ Сбросить демо</button></div></aside>
    <section className="content">
      <PreviewNavigation view={view} source={dataSource} onNavigate={changeView} />
      {view === 'backtest' ? <PreviewBacktest dataset={dataset} onBack={closeBacktest} /> : view === 'api' ? <PurchaseApiPanel recommendations={recommendations} dataset={dataset} onBack={closeBacktest} /> : <>
      <header className={`topbar ${previewStyles.topbar}`}><div><p className="breadcrumb">Электрокомплект / Закупки</p><h1>Панель пополнения</h1></div><div className={`top-actions ${previewStyles.actions}`}><span className="mode"><span /> Demo Mode</span><label className="icon-button" title="Загрузить данные"><Upload size={18} /><input type="file" accept=".xlsx,.xls,.csv" onChange={onFile} /></label><button className="button dark" onClick={runAnalysis} disabled={processing}>{processing ? <Loader2 className="spin" size={17} /> : <Sparkles size={17} />} {processing ? "Анализируем..." : "Запустить анализ"}</button></div></header>
      ...
      <AgentWorkspace dataset={dataset} selectedSku={selected?.sku ?? null} onSelectSku={sku => { const item = recommendations.find(candidate => candidate.sku === sku); if (item) setSelected(item); }} />
