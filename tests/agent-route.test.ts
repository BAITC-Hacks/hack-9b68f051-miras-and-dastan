...
describe('Explanation transport states', () => {
  it.each([null, [], 4, { context: [] }, { context: 'invalid' }])('rejects invalid request shapes: %j', async body => {
    const response = await POST(new Request('http://localhost/api/agent', { method: 'POST', body: JSON.stringify(body) }));
