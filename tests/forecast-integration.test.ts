...
describe("unchanged deterministic analytics after redesign", () => {
  ...
  it.each([
    ...
    // The agent adds explicit inventory provenance; all original calculation fields
    // must still match the pre-redesign snapshot without updating its baseline.
    const originalFields = result.map(({ inventoryKnown, ...item }) => {
      expect(inventoryKnown).toBe(true);
      return item;
    });
    expect(createHash("sha256").update(JSON.stringify(originalFields)).digest("hex")).toBe(digest);
